...
// 		m_gpuHandle.ptr += m_incrementSize;
	...
	uint32_t BatchD3D12::draw(ID3D12GraphicsCommandList* _commandList, D3D12_GPU_VIRTUAL_ADDRESS _cbv, const RenderDraw& _draw)
	{
		...
		VertexBufferD3D12& vb = s_renderD3D12->m_vertexBuffers[_draw.m_vertexBuffer[0].idx];
		vb.setState(_commandList, D3D12_RESOURCE_STATE_GENERIC_READ);

		uint16_t declIdx = !isValid(vb.m_decl) ? _draw.m_vertexDecl[0].idx : vb.m_decl.idx;
...
// 						PIX_BEGINEVENT(D3DCOLOR_DRAW, viewNameW);
				...
				if (isValid(draw.m_vertexBuffer[0]) )
				{
					...
					const VertexBufferD3D12& vb = m_vertexBuffers[draw.m_vertexBuffer[0].idx];
					uint16_t declIdx = !isValid(vb.m_decl) ? draw.m_vertexDecl[0].idx : vb.m_decl.idx;
