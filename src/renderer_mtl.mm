	...
	void RendererContextMtl::submit(Frame* _render, ClearQuad& _clearQuad, TextVideoMemBlitter& _textVideoMemBlitter) BX_OVERRIDE
	{
		...
		if (0 == (_render->m_debug&BGFX_DEBUG_IFH) )
		{
			...
			for (int32_t item = 0, restartItem = numItems; item < numItems || restartItem < numItems;)
			{
				...
				||  currentState.m_vertexBuffer[0].idx != draw.m_vertexBuffer[0].idx
				||  currentState.m_vertexDecl[0].idx != draw.m_vertexDecl[0].idx
				||  currentState.m_instanceDataStride != draw.m_instanceDataStride
				|| ( (blendFactor != draw.m_rgba) && !!(newFlags & BGFX_STATE_BLEND_INDEPENDENT) ) )
				{
					programIdx = key.m_program;
					currentState.m_vertexDecl[0]      = draw.m_vertexDecl[0];
					currentState.m_instanceDataStride = draw.m_instanceDataStride;
					...
					else
					{
						...
						if ( isValid(draw.m_vertexBuffer[0]) )
						{
							uint16_t handle = draw.m_vertexBuffer[0].idx;
							const VertexBufferMtl& vb = m_vertexBuffers[handle];
							VertexDeclHandle decl;
							decl.idx = !isValid(vb.m_decl) ? draw.m_vertexDecl[0].idx : vb.m_decl.idx;
				...
				if (currentState.m_vertexBuffer[0].idx    != draw.m_vertexBuffer[0].idx
				||  currentState.m_startVertex            != draw.m_startVertex
				||  currentState.m_instanceDataBuffer.idx != draw.m_instanceDataBuffer.idx
				||  currentState.m_instanceDataOffset     != draw.m_instanceDataOffset)
				{
					currentState.m_vertexBuffer[0]        = draw.m_vertexBuffer[0];
					currentState.m_startVertex            = draw.m_startVertex;
					currentState.m_instanceDataBuffer.idx = draw.m_instanceDataBuffer.idx;
					currentState.m_instanceDataOffset     = draw.m_instanceDataOffset;
					...
					if (invalidHandle != handle)
					{
						...
						uint16_t decl = !isValid(vb.m_decl) ? draw.m_vertexDecl[0].idx : vb.m_decl.idx;
				...
				if (isValid(currentState.m_vertexBuffer[0]) )
				{
					uint32_t numVertices = draw.m_numVertices;
					if (UINT32_MAX == numVertices)
					{
						const VertexBufferMtl& vb = m_vertexBuffers[currentState.m_vertexBuffer[0].idx];
						uint16_t decl = !isValid(vb.m_decl) ? draw.m_vertexDecl[0].idx : vb.m_decl.idx;
						const VertexDecl& vertexDecl = m_vertexDecls[decl];
